...
SELECT 
	route_id,
	COUNT(*) AS num_trips,
	percentile_disc( array[0.25,0.5,0.75] ) WITHIN GROUP ( ORDER BY sm.num_stops ) AS stop_quartiles,
	...
	( array_agg(t.trip_id) FILTER (WHERE sm.num_stops < sg.num_stops/2) )[1:4] AS bad_trips
FROM :trips_table AS t 
